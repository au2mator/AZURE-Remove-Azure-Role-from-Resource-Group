...
# Type: New Service
#
# Title: AZURE - Remove Role from Azure Resource Group
#
# v 1.0 Initial Release
#
#
# Init Release: 05.07.2022
# Last Update: 05.07.2022
# Code Template V 1.4
#
# URL: 
# Github: https://github.com/au2mator/AZURE-Remove-Azure-Role-from-Resource-Group
#################
...
[string]$LogfileName = "AZURE - Remove Role from Azure Resource Group"
...
# Stored Credentials
# See: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AZURE_RemoveResourceGroupRole&utm_content=PS1
...
#TeamsCredentials to send Teams Card #https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AZURE_RemoveResourceGroupRole&utm_content=PS1
...
try {
    ...
    try {
        ...
        try {
            ...
            $URLMember = "https://graph.microsoft.com/v1.0/users/$c_User"
            ...
            try {
                ...
                $URL = "https://management.azure.com/subscriptions/$c_subscription/resourceGroups/$c_ResourceGroup/providers/Microsoft.Authorization/roleAssignments?`$filter=principalId eq '$($ResultUser.id)'&api-version=$apiversionRole"
                $Assignments = Invoke-RestMethod -Method GET -URI $URL -headers $headers 

                #Get Role Defintion from Azure
                $URL = "https://management.azure.com/subscriptions/$c_subscription/resourceGroups/$c_ResourceGroup/providers/Microsoft.Authorization/roleDefinitions?`$filter=roleName eq '$c_Role'&api-version=$apiversionRole"
                $Roles = Invoke-RestMethod -Method GET -Uri $URL -Headers $headers

                Write-au2matorLog -Type INFO -Text "Go through each assignment"

                foreach ($Assignment in $Assignments) {
                    #Check each Assignment if it fits, than delete
                    if ($Assignment.value.properties.roleDefinitionId -eq $($roles.value.id)) {
                        Write-au2matorLog -Type INFO -Text "Assignment found and remove"
                        $URL = "https://management.azure.com$($Assignment.value.id)?api-version=$apiversionRole"
            ...
            <br>
                ...
                Write-au2matorLog -Type ERROR -Text "Error to reomve Role from Resource Group for User"
                Write-au2matorLog -Type ERROR -Text $Error
            
                $au2matorReturn = "Error to reomve Role from Resource Group for User, Error: $Error"
                $TeamsReturn = "Error to reomve Role from Resource Group for User" #No Special Characters allowed
                $AdditionalHTML = "Error to reomve Role from Resource Group for User
                <br>
                Error: $Error
                    "
                $Status = "ERROR"
